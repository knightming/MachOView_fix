...
- (void)handleThreadStateChanged:(NSNotification *)notification
{
    if ([[NSThread currentThread] isMainThread]) {
        [self doHandleThreadStateChanged:notification];
    } else {
        __weak typeof(self) weakSelf = self;
        dispatch_async(dispatch_get_main_queue(), ^{
            [weakSelf doHandleThreadStateChanged:notification];
        });
    }
}

- (void)doHandleThreadStateChanged:(NSNotification *)notification {
    if ([notification object] == dataController)
    {
